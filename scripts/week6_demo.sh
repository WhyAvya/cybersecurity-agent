...
docker run --rm -v "$PWD:/work" -w /work --user root -e OLLAMA_BASE_URL=http://host.docker.internal:11434 -e OLLAMA_MODEL=qwen2.5-coder:7b "$IMAGE_NAME" scan examples/vulnerable_app --output-dir "$ARTIFACT_ROOT/demo_vulnerable"
docker run --rm -v "$PWD:/work" -w /work --user root -e OLLAMA_BASE_URL=http://host.docker.internal:11434 -e OLLAMA_MODEL=qwen2.5-coder:7b "$IMAGE_NAME" scan examples/safe_app --output-dir "$ARTIFACT_ROOT/demo_safe"
